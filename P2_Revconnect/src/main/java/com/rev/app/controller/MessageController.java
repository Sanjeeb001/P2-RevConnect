...
public class MessageController {
    ...
    private final com.rev.app.service.PostService postService;

    public MessageController(MessageService messageService, UserService userService,
            NotificationService notificationService, com.rev.app.service.PostService postService) {
    ...
    @PostMapping("/share")
    @ResponseBody
    public Object sharePost(@RequestParam Long postId,
        ...
        // Register the share so the share count increments
        try {
            postService.sharePost(postId, currentUser);
        } catch (Exception e) {
            // Might already be shared or some error, ignore for messaging
        }

        String requestedWith = request.getHeader("X-Requested-With");
        if ("XMLHttpRequest".equals(requestedWith)) {
            // Because of Hibernate 1st level caching, the count might not update instantly
            // in findById
            return java.util.Map.of("success", true, "message", "Post shared via message successfully!", "postId",
                    postId);
        }
