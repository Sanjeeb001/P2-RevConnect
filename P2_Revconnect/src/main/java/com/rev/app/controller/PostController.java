...
public class PostController {
    ...
    @PostMapping("/{id}/like")
    @ResponseBody
    public Object likePost(@PathVariable Long id,
            @AuthenticationPrincipal UserDetails userDetails,
        ...
        if ("XMLHttpRequest".equals(requestedWith)) {
            boolean isLiked = interactionService.isLiked(currentUser.getId(), id);
            long likeCount = interactionService.getLikeCount(id);
            return new com.rev.app.dto.LikeResponseDTO(isLiked, likeCount);
        }

        String referer = request.getHeader("Referer");
        if (referer != null && !referer.isEmpty()) {
            return "redirect:" + referer; // Not an AJAX request, redirect back
        }
        return "redirect:/posts/" + id;
    }
